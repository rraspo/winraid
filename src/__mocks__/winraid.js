...
export function createWinraidMock(overrides = {}) {
  return {
    ...
    queue: {
      list: vi.fn().mockResolvedValue([]),
      stats: vi.fn().mockResolvedValue({ lifetimeCompleted: 0 }),
      reduceCompleted: vi.fn().mockResolvedValue({ lifetimeCompleted: 0 }),
    ...
    remote: {
      ...
      trashCheck: vi.fn().mockResolvedValue({ ok: true }),
      trashList: vi.fn().mockResolvedValue({ ok: true, entries: [] }),
      ...
      onFfmpegDownloadProgress: vi.fn().mockReturnValue(() => {}),
      locateFfmpeg: vi.fn().mockResolvedValue({ ok: true, path: 'C:/tools/ffmpeg.exe' }),
      ...overrides.remote,
    },
  }
}
